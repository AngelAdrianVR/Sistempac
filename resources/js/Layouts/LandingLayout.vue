...
// Reactive state for the currently active section in the viewport
const activeSection = ref('hero');

// Updated navigation links to correspond with section IDs
const navLinks = [
    { name: 'Inicio', href: '#hero' },
    { name: 'Calidad', href: '#calidad' },
    { name: 'Productos', href: '#variedad' },
...
onMounted(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        rootMargin: '-50% 0px -50% 0px', // Trigger when the vertical center of the section hits the vertical center of the viewport
        threshold: 0 
    });
                ...
                <div class="flex items-center justify-between h-20">
                    ...
                    <div class="flex-shrink-0">
                        <Link href="/" class="text-2xl font-bold">
                            <img class="w-80 md:w-full" src="/images/logo_propack.png" alt="propack">
                        </Link>
                    </div>
                    ...
                    <nav class="hidden md:flex items-center space-x-8">
                        <a v-for="link in navLinks" 
                           ...
                           :class="{ 'text-amber-500 font-semibold': activeSection === link.href.substring(1) }">
                            {{ link.name }}
                            <span v-if="activeSection === link.href.substring(1)" class="absolute -bottom-2 left-1/2 -translate-x-1/2 w-2 h-2 bg-amber-500 rounded-full"></span>
                        </a>
                        <!-- External link to the store -->
                    ...
                    <div class="hidden md:flex items-center">
                         <div class="relative">
                            <input type="text" placeholder="Buscar productos..." class="w-48 lg:w-64 pl-10 pr-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all duration-300">
                ...
                <div v-if="isMobileMenuOpen" class="md:hidden" id="mobile-menu">
                    <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                         <div class="relative px-2 pb-4">
                            <input type="text" placeholder="Buscar productos..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-amber-500">
                        ...
                        <a v-for="link in navLinks" 
                           ...
                           :class="activeSection === link.href.substring(1) ? 'bg-amber-500 text-white' : 'text-gray-600 hover:bg-gray-100'">
                            {{ link.name }}
                        </a>
        ...
        <footer class="bg-slate-800 text-gray-300">
            <div class="container mx-auto py-12 px-4 sm:px-6 lg:px-8">
                        ...
                        <h4 class="font-semibold text-white tracking-wider uppercase mb-4">{{ footerSections.enlaces.title }}</h4>
                         <ul class="space-y-2">
                             <li v-for="item in footerSections.enlaces.items" :key="item.text">
                                 <a :href="item.href" class="text-sm hover:text-amber-500 transition-colors">{{ item.text }}</a>
                             </li>
                         </ul>
                    </div>
                ...
                <div class="mt-8 border-t border-gray-700 pt-6 text-center text-sm text-gray-400">
                    <p>&copy; {{ new Date().getFullYear() }} PROPACK. Todos los derechos reservados.</p>
                </div>
            </div>
        </footer>
    </div>
